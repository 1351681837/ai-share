<template>
  <el-container class="my-table-container">
    ...
    <el-table :data="tableData" height="70vh" style="width: 100%">
      ...
      <el-table-column prop="cate.title" label="视频分类"/>
      <el-table-column label="视频">
        <template #default="scope">
          <div style="display: flex; align-items: center">
            <video :src="scope.row.video_src"></video>
          </div>
        </template>
      </el-table-column>
      <el-table-column label="视频封面">
        <template #default="scope">
          <div style="display: flex; align-items: center">
            <el-avatar shape="square" :size="100" fit="fill" :src="scope.row.cover_url"/>
          </div>
        </template>
      </el-table-column>
      ...
      <el-table-column prop="updated_at" label="排序"/>
...
import {getVideoTutorialList} from "../../common/api";
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.$router.push('/video-tutorial-edit')
    },
    handleEdit(id) {
    ...
    getData() {
      ...
      getVideoTutorialList(this.search).then((res) => {
        let data = this.$data;
